...
with open('/home/wangkun/scPhyloX/datasets/cellnumber_sample1/computational_times.txt', 'a') as f:
    data = pickle.load(gzip.open(f'/home/wangkun/scPhyloX/datasets/cellnumber_sample1/{i}', 'rb'))
    seqtab_full = np.array([i.seq for i in data['NC']] + [i.seq for i in data['AC']])
    ...
    for sr in [1, 0.7, 0.5, 0.2]:
        ...
        time1 = time()
        pickle.dump(idata_bl, gzip.open(f"/home/wangkun/scPhyloX/datasets/cellnumber_sample1/{i.replace('.pkl.gz', f'_idata_bl_{sr}.pkl')}", 'wb'))
        time2 = time()
        ...
        time3 = time()
        pickle.dump(idata, gzip.open(f"/home/wangkun/scPhyloX/datasets/cellnumber_sample1/{i.replace('.pkl.gz', f'_idata_mn_{sr}.pkl')}", 'wb'))
